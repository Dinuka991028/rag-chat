...
public class LlmChatService implements ChatService {
    ...
    public String askAIWithContext(String prompt, String role) {
        ...
        if ("officer".equals(effectiveRole)) {
            String reportReply = tryBuildOfficerStatusReport(safePrompt);
            if (reportReply != null) {
                return reportReply;
            }
        }
    ...
    public ChatConversationResponse askAIWithContextAndHistory(String conversationId, String message, String role) {
        ...
        if ("officer".equals(effectiveRole)) {
            String reportReply = tryBuildOfficerStatusReport(safeMessage);
            if (reportReply != null) {
                conversationHistoryService.append(id, safeMessage, reportReply);
                return new ChatConversationResponse(id, reportReply);
            }
        }
    ...
    private String tryBuildOfficerStatusReport(String query) {
    ...
    static boolean isOfficerStatusCountIntent(String query) {
        ...
        String q = query.toLowerCase(Locale.ROOT);
        boolean hasCountWord = q.contains("how many")
                || q.contains("count")
                || q.contains("report")
                || q.contains("summary")
                || q.contains("stats")
    ...
    private Map<String, Integer> countOfficerTaskStatuses() {
        List<ai_chat.domain.OfficerKnowledgeDocument> rows = officerKnowledgeDocumentRepository.findAll();
        if (rows == null || rows.isEmpty()) {
            return Map.of();
        }
        Map<String, Integer> counts = new LinkedHashMap<>();
        for (ai_chat.domain.OfficerKnowledgeDocument row : rows) {
    ...
    static String extractTaskStatusFromContent(String content) {
        ...
        Matcher m = Pattern.compile("(?im)^Task\\s+Status\\s*:\\s*(.+)$").matcher(content);
        if (!m.find()) {
            return null;
        }
