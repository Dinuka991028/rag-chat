...
import ai_chat.routing.OfficerQueryRoutingService;
...
public class LlmChatService implements ChatService {
    ...
    @Autowired
    private OfficerQueryRoutingService officerQueryRoutingService;
    ...
    public String askAIWithContext(String prompt, String role) {
        ...
        // Knowledge base emptiness check should be role-aware; officer lookups use a different collection.
        if ("officer".equals(effectiveRole)) {
            if (officerKnowledgeDocumentRepository.count() == 0) {
        ...
        if ("officer".equals(effectiveRole)) {
            String deterministicReply =
                    officerQueryRoutingService.tryRouteDeterministicReply(safePrompt, effectiveRole);
            if (deterministicReply != null) {
                deterministicReply = applyOutboundGovernance(safePrompt, deterministicReply);
    ...
    public ChatConversationResponse askAIWithContextAndHistory(String conversationId, String message, String role) {
        ...
        // Role-aware emptiness check; officer lookups use a different collection.
        if ("officer".equals(effectiveRole)) {
            if (officerKnowledgeDocumentRepository.count() == 0) {
